...
:G4VUserDetectorConstruction(),
 fLogicTarget(NULL), fLogicChamber(NULL), fLogicChamber2(NULL),
...
G4VPhysicalVolume* B2bDetectorConstruction::DefineVolumes()
{
  ...
  // Always return the physical world
    ...
    /*-------------------------------------------------------------------------
---------------- Dimensiones y posición de Geometrías  ----------------------
-----------------      fuera del detector  ----------------------------------*/

    //     
    // Cono truncado
    // posiciónamiento 
    G4ThreeVector pos1 = G4ThreeVector(0, 0*cm, 500*cm);

  
            
    // Conical section shape       
    G4double shape1_rmina =  0.*cm, shape1_rmaxa = 20.*cm;
    G4double shape1_rminb =  0.*cm, shape1_rmaxb = 70.*cm;
    G4double shape1_hz = 30.*cm;
    G4double shape1_phimin = 0.*deg, shape1_phimax = 360.*deg;

    G4Cons*  solidCone =    
        new G4Cons("Solid-Cone", 
    ...
    fLogicChamber2 =                         
        new G4LogicalVolume(solidCone,         //its solid
                            air,         //its material
                            "Logical-Cone");   //its name
                
    new G4PVPlacement(0,            //no rotation
                        pos1,              //at position
                        fLogicChamber2,         //its logical volume
                        "Physical-Cone",   //its name
                        worldLV,        //its mother  volume
                        false,             //no boolean operation
                        0,                 //copy number
                        true);             //overlaps checking
  
  G4VisAttributes* color = new G4VisAttributes(G4Colour(1.0,0.6,0.0));
  fLogicChamber2->SetVisAttributes(color);


  G4double maxStep = 100*cm;
  //G4double maxStep = 0.5*chamberWidth;
  fStepLimit = new G4UserLimits(maxStep);
  fLogicChamber2->SetUserLimits(fStepLimit);
...
void B2bDetectorConstruction::ConstructSDandField()
{
  ...
  SetSensitiveDetector( fLogicChamber2,  aTrackerSD );
