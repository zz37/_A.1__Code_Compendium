...
// HODOSCOPIOS BRAZO 1 Y 2 --> NARANJA.   
...
//DRIFTCHAMBER 1 Y 2 AZUL-MORADO
...
// SON LOS MISMO QUE DEL DRIFT CHAMBER PERO LE DA UN GROSOR DE EN MM, ESTE ES EL SENSIBLE
// MÁS CHICO QUE LOS ORIGINALES DE ARRIBA
  // "virtual" wire plane
...
// CALORIMETER GRANDE BLOQUE --> BLANCO  
  // hadron calorimeter
  ...
  new G4PVPlacement(0,G4ThreeVector(0.,0.,3.*m),hadCalorimeterLogical,
                    ...
                    false,0,checkOverlaps);
// CALORIMETER GRANDE BLOQUE COLUMNAS --> ROJO    
...
// CALORIMETER GRANDE BLOQUE horizontales --> ROJO    
...
// CALORIMETER GRANDE BLOQUE layes horizontales y verticales --> ROJO    
...
// CALORIMETER GRANDE BLOQUE layers horizontales y verticales de cintillador --> ROJO     
  // scintillator plates
...
// esto es para el mundo de color cyan
  auto visAttributes = new G4VisAttributes(G4Colour(0,1,1));
  visAttributes->SetVisibility(false);
  worldLogical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);

// esto es para los volumenes de los dos brazos arm 1 y arm 2 
  visAttributes = new G4VisAttributes(G4Colour(1,0,1));
  visAttributes->SetVisibility(false);
  firstArmLogical->SetVisAttributes(visAttributes);
  secondArmLogical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);

// este para colorear el cilíindro a color amarillo
  visAttributes = new G4VisAttributes(G4Colour(1,1,0));  
  fMagneticLogical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes); 
  
// COLOR DE LOS HODOSCOPIO --> NARANJA. PARA BRAZO 1 Y 2
  visAttributes = new G4VisAttributes(G4Colour(1,0.6,0));
  fHodoscope1Logical->SetVisAttributes(visAttributes);
  fHodoscope2Logical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);
  
// COLOR DE CHAMBERDRIFTS -> AZUL-MORADO
  visAttributes = new G4VisAttributes(G4Colour(0.3,0.1,1));
  chamber1Logical->SetVisAttributes(visAttributes);
  chamber2Logical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);

// COLOR DE GROSOR SÓLIDOS DE CHAMBERDRIFTS -> ROJO
  visAttributes = new G4VisAttributes(G4Colour(1,0,0));
  visAttributes->SetVisibility(false);
  fWirePlane1Logical->SetVisAttributes(visAttributes);
  fWirePlane2Logical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);

// COLOR DE CALORIMETER ANTES DE PARTICIÓN EN CELLS --> ROJO
  visAttributes = new G4VisAttributes(G4Colour(1,0,0));
  visAttributes->SetVisibility(false);
  emCalorimeterLogical->SetVisAttributes(visAttributes);
  fVisAttributes.push_back(visAttributes);

// COLOR DE CELDAS DE CALORIMETER  --> VERDE ASQEUROSO
  visAttributes = new G4VisAttributes(G4Colour(0.5,1,0));
...
// // CALORIMETER GRANDE BLOQUE, COLUMNAS, FILAS Y COLS-FILS Y COLS-FILS DE CINTILLEO
  visAttributes = new G4VisAttributes(G4Colour(1,0,0));
  visAttributes->SetVisibility(false);
...
/* 
// WIREPLANE ES SENSIBLE ES EL MINI GROSOR DEL DRIFTCHAMBERS 1
...
/* 
// CELDAS CALORIMETER ES SENSIBLE --> SON LOS VERDE ASQUEROSO  
