...
void B5EventAction::BeginOfEventAction(const G4Event*)
{
  ...
  if (fHodHCID[0] == -1) {
    ...
    array<G4String, kDim> hHCName  = { "hodoscope1/hodoscopeColl"};
    

    G4cout << "POPOPOADAASDFAD"  <<G4endl;
    G4cout << "hHCName ::: :" << hHCName[0] <<G4endl;


    
// SON PARA EL DRIFTCHAMBERS Y CALORIMETERS
//   array<G4String, kDim> dHCName = {{ "chamber1/driftChamberColl", "chamber2/driftChamberColl" }};
//   array<G4String, kDim> cHCName = {{ "EMcalorimeter/EMcalorimeterColl", "HadCalorimeter/HadCalorimeterColl" }};

    // histograms names
//    array<array<G4String, kDim>, kDim> histoName = {{ {{ "Chamber1", "Chamber2" }}, {{ "Chamber1 XY", "Chamber2 XY" }} }};

    for (G4int iDet = 0; iDet < kDim; ++iDet) {
      // hit collections IDs
      G4cout << "POPO GRANDE DENTRO FOR :" <<G4endl;
      G4cout << "ANTED de POPO GRANDE fHodHCID[0] :" << fHodHCID[0]<<G4endl;
      fHodHCID[iDet]   = sdManager->GetCollectionID(hHCName[iDet]);
      G4cout << "DESPUES POPO GRANDE fHodHCID[0] :" << fHodHCID[0]<<G4endl;
